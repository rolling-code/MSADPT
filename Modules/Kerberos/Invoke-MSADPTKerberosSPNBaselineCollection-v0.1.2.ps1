...
try {
    ...
    # Validate the selected server by querying RootDSE directly. Do not combine
    # Get-ADDomainController -Identity and -Server with the same FQDN because
    # valid DC hostnames can fail identity resolution even while ADWS/LDAP works.
    $RootDseParameters = @{
        Server = $Server
        ErrorAction = 'Stop'
    }
    if ($null -ne $Credential) { $RootDseParameters.Credential = $Credential }

    Write-Step 'NETWORK' ("Target={0}; ports=TCP/9389 or environment ADWS/LDAP; protocol=ActiveDirectory module; operation=read-only RootDSE validation" -f $Server) DarkCyan
    $ValidatedRootDse = Get-ADRootDSE @RootDseParameters
    $ValidatedNamingContext = Get-FirstTextValue $ValidatedRootDse.defaultNamingContext
    if ($null -eq $ValidatedNamingContext -or $ValidatedNamingContext.Length -eq 0) {
        throw 'Domain-controller RootDSE validation returned no default naming context.'
    }
